...
originally sat in `clau-dom/` as a zip; it was moved here on 16 July 2026 because it is
Hermes-side tooling and because it is the direct design input for Phase 5 of
`../../DEVELOPMENT_PLAN.md` (the distillation loop):

- `hermd distill <trace.jsonl>` will follow this skill's record, reproduce, verify, stage,
...
- The watcher plugin's threshold pattern (tool calls, external hits, wall clock) maps to
  flagging any `hermd` run over N steps as a distillation candidate.
