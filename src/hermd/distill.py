...
from hermd.trace import SCRUBBED
...
def selector_for(element: dict[str, Any]) -> str:
    ...
    raise DistillError(
        ...
        "Re-run the flow with a current hermd so the trace captures elements."
    )


def _secret_name(element: dict[str, Any], used: set[str]) -> str:
    base = element.get("name") or element.get("id") or "value"
    name = "HERMD_SECRET_" + re.sub(r"[^A-Za-z0-9]+", "_", base).strip("_").upper()
    candidate, n = name, 2
...
Generated by `hermd distill`. This is a plain Playwright script: no model, no
